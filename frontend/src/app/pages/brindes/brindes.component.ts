...
@Component({
            ...
            @for (b of brindes(); track b.id) {
              ...
              <tr>
                <td [attr.colspan]="authService.isAdmin() ? 9 : 8" class="empty-state">
                  Nenhum item de brinde cadastrado.
                </td>
              </tr>
            }
          </tbody>
        ...
        <div class="modal-backdrop" (click)="closeModal()">
          <div class="modal-content" (click)="$event.stopPropagation()">
            <div class="modal-header">
              <h2>{{ isEditing() ? 'Editar Brinde / Item' : 'Novo Brinde / Item' }}</h2>
            ...
            <form (ngSubmit)="saveBrinde()">
              <div class="form-group">
                ...
                <input type="text" class="form-control" [(ngModel)]="formData.item" name="item" required placeholder="Ex: Camiseta AWS Student Builder (Tam M)">
              </div>

              <div class="form-group">
                <label class="form-label">Origem (Parceria Vinculada)</label>
                <select class="form-select" [(ngModel)]="formData.origemParceriaId" name="origemParceriaId">
                  <option [ngValue]="null">Próprio / Fabricação do Grupo</option>
              ...
              <div class="form-row">
                <div class="form-group flex-1">
                  ...
                  <input type="number" min="0" class="form-control" [(ngModel)]="formData.qtdRecebida" name="qtdRecebida" required placeholder="0">
                </div>

                <div class="form-group flex-1">
                  <label class="form-label">Qtd Distribuída *</label>
                  <input type="number" min="0" class="form-control" [(ngModel)]="formData.qtdDistribuida" name="qtdDistribuida" required placeholder="0">
                </div>
              </div>

              <div class="stock-preview">
                Saldo restante em estoque: <strong>{{ ((formData.qtdRecebida || 0) - (formData.qtdDistribuida || 0)) }} un</strong>
              </div>
              ...
              <div class="form-row">
                <div class="form-group flex-1">
                  ...
                  <select class="form-select" [(ngModel)]="formData.eventoDistribuicaoId" name="eventoDistribuicaoId">
                    <option [ngValue]="null">Não definido / Múltiplos</option>
                    @for (ev of eventos(); track ev.id) {
                      <option [ngValue]="ev.id">{{ ev.nome }}</option>
                    }
                  </select>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" [(ngModel)]="formData.observacoes" name="observacoes" rows="2" placeholder="Local de armazenamento, tamanhos, etc."></textarea>
              </div>
  ...
  styles: [`
    ...
    .pill {
      ...
      &.pill-purple { border-color: var(--color-purple); strong { color: #7B1BE0; } }
    ...
    .stock-count {
      ...
      &.stock-zero { background: var(--color-amber-subtle); color: #B36B00; }
...
export class BrindesComponent implements OnInit {
  ...
  totalRecebido(): number {
    return this.brindes().reduce((acc, curr) => acc + (Number(curr.qtdRecebida) || 0), 0);
  }

  totalDistribuido(): number {
    return this.brindes().reduce((acc, curr) => acc + (Number(curr.qtdDistribuida) || 0), 0);
  }

  totalSaldoEstoque(): number {
    return this.totalRecebido() - this.totalDistribuido();
  }
  ...
  editBrinde(b: Brinde) {
    ...
    this.formData = {
      item: b.item,
      origemParceriaId: b.origemParceriaId,
      qtdRecebida: b.qtdRecebida,
      qtdDistribuida: b.qtdDistribuida,
      eventoDistribuicaoId: b.eventoDistribuicaoId,
      dataDistribuicao: b.dataDistribuicao,
  ...
  deleteBrinde(id: number) {
    if (confirm('Tem certeza que deseja excluir este brinde do estoque?')) {
